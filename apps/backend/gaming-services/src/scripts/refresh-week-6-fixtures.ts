...
async function refreshWeek6Fixtures() {
  ...
  try {
    console.log('🔄 Starting Week 6 fixtures refresh...');

    // Step 1: Delete existing week 6 fixtures and related data
    console.log('🗑️  Deleting existing week 6 data...');

    // First, get all week 6 fixture IDs
    const week6Fixtures = await dataSource.query(
      `SELECT id FROM fixtures WHERE week = 6`,
    );

    if (week6Fixtures.length > 0) {
      const fixtureIds = week6Fixtures.map((f: any) => f.id);
      ...
      const fixturesResult = await dataSource.query(
        `DELETE FROM fixtures WHERE week = 6`,
      );
      console.log(
        `  - Deleted ${fixturesResult.rowCount || 0} week 6 fixtures`,
      );
    } else {
      console.log('  - No week 6 fixtures found to delete');
    }

    // Step 2: Fetch week 6 fixtures from API
    console.log('📡 Fetching Round 6 fixtures from API-Football...');

    const baseUrl = apiHost.startsWith('http') ? apiHost : `https://${apiHost}`;
    const url = `${baseUrl}/fixtures?league=135&season=2025&round=${encodeURIComponent('Regular Season - 6')}`;
    ...
    console.log('🎉 Week 6 fixtures refresh completed successfully!');

    // Show summary
    const countResult = await dataSource.query(
      `SELECT COUNT(*) as count FROM fixtures WHERE week = 6`,
    );
    console.log(
      `📊 Total week 6 fixtures in database: ${countResult[0].count}`,
    );
  } catch (error) {
    console.error('❌ Error refreshing week 6 fixtures:', error);
    if (error.response) {
...
// Run the script
refreshWeek6Fixtures().catch((error) => {
